    ...
    form.addEventListener("submit", function (event) {
        ...
        const callbackForUpdate = (responseStatus, responseData) => {
            ...
            if (responseStatus == 200) {
                window.location.href = `singleUserMessage.html?user_id=${user_id}`;
            } else {
                alert(responseData.message);
            }
        };

        const message_text = document.getElementById("update_message_text").value;
        const data = {
            message_text: message_text,
        };
        fetchMethod(currentUrl + `/api/message/${message_id}/${user_id}`, callbackForUpdate, "PUT", data, localStorage.getItem("token"));
    });
});
